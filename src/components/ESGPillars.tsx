...
import { Shield, HeartHandshake, Leaf, TrendingUp, Info } from 'lucide-react';
...
export const ESGPillars: React.FC = () => {
  ...
  return (
    <section id="etica-cidadania-sustentabilidade" style={{ padding: '4rem 2rem', borderBottom: '1px solid var(--border-subtle)' }}>
      <SectionHeader
        tag="TÓPICO 5"
        title="Ética, Cidadania e Sustentabilidade"
        subtitle="O modelo de negócios ESG da WEG sustentado no Triple Bottom Line, equilibrando retorno financeiro, desenvolvimento comunitário e impacto ecológico regenerativo."
        accentColor="#10b981"
      />

      {/* Triple Bottom Line Grid */}
      <div style={{
        display: 'grid',
        ...
        {pillars.map((p, idx) => (
          <div key={idx} className="glass-panel" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <h4 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#ffffff' }}>{p.title}</h4>
            </div>
            ...
            <div style={{
              ...
              <span style={{ fontSize: '1rem', fontWeight: 700, color: p.color }}>{p.stats}</span>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 500 }}>{p.subStats}</span>
            </div>
          </div>
        ))}
      </div>

      {/* SBTi Decarbonization Roadmap */}
      <div className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--success)', marginBottom: '0.75rem' }}>
          ...
          <span style={{ fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
            Trajetória Científica de Descarbonização
          </span>
        </div>
        
        <h4 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '1.5rem', color: '#ffffff' }}>
          Planejamento de Emissões de Carbono (Validação SBTi)
        </h4>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <svg viewBox="0 0 500 200" width="100%" height="auto" style={{ overflow: 'visible' }}>
              ...
              <text x="450" y="150" fill="#ffffff" fontSize="11" textAnchor="middle" fontWeight="700">NET-ZERO (100%)</text>
            </svg>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {!hoveredPoint ? (
                <div>
                  ...
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                    Passe o mouse ou toque nos pontos do gráfico para revelar os detalhes operacionais de cada marco de emissões validados pela <strong>Science Based Targets initiative (SBTi)</strong>.
                  </span>
                </div>
              ) : hoveredPoint === 'current' ? (
                <div>
                  <span style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--success)', display: 'block', marginBottom: '0.25rem' }}>Status Atual: -31,9% nas Emissões</span>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                    Resultados auditados em 2025 para emissões de Escopo 1 (diretas da produção) e Escopo 2 (energia adquirida). Indica a substituição massiva por fontes renováveis e ganhos térmicos internos.
                  </span>
                </div>
              ) : hoveredPoint === '2030' ? (
                <div>
                  <span style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--success)', display: 'block', marginBottom: '0.25rem' }}>Meta Intermediária 2030: -52%</span>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                    Compromisso robusto e intermediário de redução absoluta de emissões de carbono operacionais. Uma das metas industriais mais agressivas e validadas pela SBTi na América Latina.
                  </span>
                </div>
              ) : (
                <div>
                  <span style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--success)', display: 'block', marginBottom: '0.25rem' }}>Fronteira 2050: Net-Zero Total</span>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                    Descarbonização absoluta de toda a cadeia produtiva (incluindo fornecedores e logística - Escopo 3), neutralizando qualquer impacto de carbono residual no planeta.
                  </span>
                </div>
              )}
            </div>
            ...
            <div style={{
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: '1.4' }}>
                <strong>Validação Científica:</strong> A SBTi (Science Based Targets initiative) garante que a curva de redução da WEG está alinhada às metas do Acordo de Paris para limitação do aquecimento global a 1.5°C.
              </span>
            </div>
          </div>
        </div>
